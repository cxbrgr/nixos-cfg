...
## List all detected WD Passport devices with details
def list_devices():
	context = pyudev.Context()
	passport_devices = []
	
	for disk_device in context.list_devices(subsystem='block', DEVTYPE='disk'):
		# Skip CD-ROM devices (sr*)
		if disk_device.sys_name.startswith('sr'):
			continue
	...
	print("Western Digital Passport devices found:")
	print()
	for device in passport_devices:
		serial = device.properties.get("ID_SERIAL_SHORT", "unknown")
		...
		if size:
			size_gb = int(size) * 512 / (1024**3)
			size_str = "{:.1f} GB".format(size_gb)
		else:
			size_str = "unknown"
		
		print("  Device:  {}".format(device.device_node))
		print("  Vendor:  {}".format(vendor))
		print("  Model:   {}".format(model))
		print("  Serial:  {}".format(serial))
		print("  Size:    {}".format(size_str))
		print()
...
def main(argv): 
	...
	parser.add_argument("-l", "--list", required=False, action="store_true", help="List all detected WD Passport devices")

	args = parser.parse_args()
	
	# Handle --list early, before device selection
	if args.list:
		list_devices()
		sys.exit(0)
	...
	if len(passport_devices) == 0:
		...
		print("Multiple Western Digital Passport devices found:")
		for device in passport_devices:
			print(" - {}".format(device.device_node))
		print(fail("Use --device /dev/___ to choose."))
		sys.exit(1)
	...
	if args.change_passwd:
		...
		change_password()
	if args.erase:
		print(question("All data on {} will be lost. Confirm deletion by typing 'delete' and pressing enter.".format(
			device_name
		)))
		r = sys.stdin.read(6)
		if r.lower() == 'delete':
